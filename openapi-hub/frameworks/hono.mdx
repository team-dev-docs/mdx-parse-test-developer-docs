...
When building complex schemas, use `.merge()` instead of `.and()` to combine schemas. Using `.and()` typically results in a union type in the OpenAPI schema, while `.merge()` creates a flatter, more maintainable schema structure.
