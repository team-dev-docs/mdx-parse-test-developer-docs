...
<div data-tabs data-items="['TypeScript', 'Go']">
...
As an example, in Node.js v20, streaming a large file to a server using an SDK is only a handful of lines. In the browser,
...
- **Browsers:** All supported browsers return an instance of a [`File`](https://developer.mozilla.org/en-US/docs/Web/API/File) when reading the value from an `<input type="file">` element.
